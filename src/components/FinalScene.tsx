...
const COLORS = ['#c97b84', '#e8b4b8', '#9a7a9e', '#7d6b8a', '#fdf8f2', '#d4a8b4', '#f5ebe3']
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {particles.map((p) => (
        <motion.div
          key={p.id}
          initial={{ x: `${p.x}vw`, y: `${p.y}vh`, rotate: p.angle, opacity: p.opacity }}
          animate={{
            y: '110vh',
        ...
        <AnimatePresence mode="wait">
          {!celebrated ? (
            <motion.div
              ...
              exit={prefersReducedMotion ? { opacity: 0 } : { opacity: 0, scale: 1.04, filter: 'blur(6px)' }}
